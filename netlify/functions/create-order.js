...
//
// Flow:
//   Card payments:  verifies Stripe PaymentIntent is 'succeeded', then saves + emails
//   Bank transfer:  saves order as 'pending', sends instructions + notification
//
...
//
// ─────────────────────────────────────────────
// SERVER-SIDE CATALOG — keep in sync with create-payment-intent.js and modals.js
// ─────────────────────────────────────────────
const CATALOG = {
  ...
  // Add new items here alongside create-payment-intent.js and window.CHECKOUT_CATALOG in modals.js
};
...
async function verifyPaymentIntent(paymentIntentId, stripeKey) {
  ...
  // Return the Stripe-verified amount as the source of truth
...
function buildConfirmationEmail({ firstName, itemTitle, itemType, variant, price, orderId, orderType, paymentMethod, sigHtml, senderEmail }) {
  ...
  const deliveryNote = isPreorder
    ? `<p style="margin:0 0 14px;font-size:14px;color:#555;line-height:1.7;">Your ${itemType === 'book' ? 'copy will be dispatched' : 'item will be delivered'} as soon as it is available. You will receive a follow-up confirmation at that time.</p>`
  ...
  const content = `
    ...
    <table style="width:100%;border-collapse:collapse;border:1px solid #e8e4dd;margin-bottom:20px;">
      ...
      <tr>
        <td style="padding:10px 14px;font-size:13px;color:#888;">Item</td>
        <td style="padding:10px 14px;font-size:14px;color:#1a1a1a;">${itemTitle}</td>
      </tr>
      <tr style="background:#f7f5f0;">
        <td style="padding:10px 14px;font-size:13px;color:#888;">Format</td>
        <td style="padding:10px 14px;font-size:14px;color:#1a1a1a;">${variant}</td>
      </tr>
      <tr>
    ...
    <div style="border-top:1px solid #f0ede6;padding-top:20px;margin-top:24px;">
      ...
      <p style="margin:0 0 14px;font-size:13px;color:#555;line-height:1.6;">
        ${isPreorder
          ? 'Pre-orders can be cancelled at any time before fulfilment for a full refund. Simply reply to this email with your order number.'
          : 'To request a return or refund, please reply within 14 days of purchase with your order number.'}
      </p>
      <a href="mailto:${senderEmail}?subject=Cancel%20order%20${orderId}"
    ...
    <div style="border-top:1px solid #f0ede6;padding-top:20px;margin-top:20px;">
      <p style="margin:0 0 10px;font-size:13px;color:#888;">While you wait</p>
      <a href="https://phelim.me/podcast.html"
         style="display:inline-block;background:#263d33;color:#f8f6f1;text-decoration:none;padding:9px 18px;font-size:13px;letter-spacing:.02em;">
        Listen: Future Foundations Podcast →
      </a>
    </div>`;
...
function buildOwnerNotification({ firstName, lastName, email, itemTitle, variant, price, orderId, orderType, paymentMethod, sigHtml }) {
  const content = `
    ...
    <table style="width:100%;border-collapse:collapse;border:1px solid #e8e4dd;margin-bottom:20px;">
      ...
      <tr><td style="padding:8px 12px;font-size:13px;color:#888;">Payment</td><td style="padding:8px 12px;font-size:14px;color:#1a1a1a;">${paymentMethod === 'bank' ? 'Bank transfer (awaiting payment)' : 'Card — paid'}</td></tr>
    </table>
  ...
  return buildEmailWrapper({
    ...
    heading: `${firstName} ${lastName} ordered ${itemTitle}`,
    content, closing: null, sigHtml,
  });
}
...
exports.handler = async function(event) {
  ...
  if (!firstName || !email) return json(400, { error: 'firstName and email are required' });
  if (!isEmail(email))      return json(400, { error: 'Invalid email address' });
  if (!['card','bank'].includes(paymentMethod)) return json(400, { error: 'Invalid paymentMethod' });
  if (!['preorder','purchase'].includes(orderType)) return json(400, { error: 'Invalid orderType' });

  // ── Validate item + variant against server catalog ──────────────────────────
  const catalogItem = CATALOG[itemId];
  let finalPrice;
  let itemType = 'item';

  if (catalogItem) {
    // Known item — validate variant and use catalog price
    if (!catalogItem.variants[variant]) {
      return json(400, { error: `Unknown variant "${variant}" for this item` });
    }
    finalPrice = catalogItem.variants[variant];
    itemType   = catalogItem.type || 'item';
  } else {
    // Unknown item — not in catalog, reject
    return json(400, { error: `Item "${itemId}" is not in the catalog` });
  }

  // ── For card orders: verify Stripe PaymentIntent ─────────────────────────────
  let orderStatus = 'pending';
  if (paymentMethod === 'card') {
    if (!paymentIntentId) return json(400, { error: 'paymentIntentId is required for card orders' });
    ...
    if (!verify.ok) return json(400, { error: verify.error });

    // Use Stripe-verified price as the authoritative amount
    finalPrice  = verify.verifiedPrice;
    orderStatus = 'confirmed';
  }

  const resolvedTitle = itemTitle || catalogItem?.title || itemId;
  ...
  try {
    await sendEmail(RESEND_KEY, {
      ...
      html:    buildConfirmationEmail({
        firstName, itemTitle: resolvedTitle, itemType, variant,
        price: finalPrice, orderId, orderType, paymentMethod, sigHtml, senderEmail: SENDER,
      }),
    });
  } catch(e) { errors.push('confirmation: ' + e.message); console.error(e.message); }
  ...
  try {
    await sendEmail(RESEND_KEY, {
      ...
      html:    buildOwnerNotification({
        firstName, lastName, email, itemTitle: resolvedTitle, variant,
        price: finalPrice, orderId, orderType, paymentMethod, sigHtml,
      }),
    });
  } catch(e) { errors.push('notification: ' + e.message); console.error(e.message); }
