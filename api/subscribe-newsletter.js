// Vercel Serverless Function: subscribe-newsletter
// POST /api/subscribe-newsletter  { name, email }
...
module.exports = async function(req, res) {
  ...
  if (!SUP_URL || !SUP_KEY) { respond(res, 500, { error: 'Database not configured' }); return; }
  ...
  try {
    ...
    if (Array.isArray(rateRows) && rateRows.length > 0) { respond(res, 200, { success: true, duplicate: true }); return; }
  } catch(_) {}
  ...
  try {
    await fetch(`${SUP_URL}/rest/v1/newsletter_subscribers`, {
      ...
      body: JSON.stringify([{ id: 'ns-' + Date.now() + '-' + Math.random().toString(36).slice(2, 6), name, email, subscribed_at: new Date().toISOString(), active: true }]),
    });
  ...
  if (RESEND_KEY) {
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({ from: `Phelim Ekwebe <${SENDER}>`, to: [email], subject: "Welcome — you're subscribed to Future Foundations", html: buildWelcomeHtml(name) }),
      });
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0ede6;padding:40px 20px;">
  <tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    ...
    <tr><td style="padding:20px 40px;text-align:center;">
      <p style="margin:0;font-size:11px;color:#999;line-height:1.6;">You subscribed at <a href="https://phelim.me" style="color:#263d33;">phelim.me</a>.<br>&copy; ${new Date().getFullYear()} Phelim Ekwebe.</p>
    </td></tr>
  </table></td></tr>
</table></body></html>`;
}
